...
def evaluate_power_on_turn(hand: list, table_cards: list, deck: Deck):
    ...
    for card in available_cards:
        current_table = table_cards + [card]
        player_hand_power = evaluate_hand(hand, current_table)
        for villain_hand in available_2_cards:
            if card != villain_hand[0] and card != villain_hand[1]:
                hand_power = evaluate_hand(list(villain_hand), current_table)
...
evaluate_power_on_turn(player.get_cards(), table_cards, deck)
end_time = time.time()
